...
export const isFileMp3 = (filename: string) => {
	const arr =
		".mp3, .mp4, .wma, .amr, .m4a, .wav, .3gp, .aac, .ogg-opus, .flac".split(
			/[, ]+/gi,
		);
	mlog("fileIsMp3", arr);
...
export const getLastVersion = async () => {
	const url =
		"https://api.github.com/repos/Dooy/chatgpt-web-midjourney-proxy/tags?per_page=1";
...
export const canVisionModel = (model: string) => {
	...
	if (["gpt-4-all", "gpt-4-v", "gpt-4v", "gpt-3.5-net"].indexOf(model) > -1)
		return true;
...
export const loadGallery = async () => {
	...
	const ids = list
		.filter((v) => isStringOnlyDigits(v.reqid))
		.map((v) => +v.reqid);
	mlog("ids", ids);
