...
function create() {
	...
	homeStore.setMyData({ showDrawDrawer: false });
}
...
function createPrompt(rz: string) {
	...
	for (let v of farr) {
		...
		if (v.k === "quality") rz += `  --q ${f.value.quality}`;
			...
			if (f.value.styles) rz += ` ${f.value.styles}`;
		} else if (v.k === "version") {
			st.value.bot = "";
...
function selectFile(input: any) {
	...
	upImg(input.target.files[0])
		.then((d) => st.value.fileBase64.push(d))
...
const exportToTxt = async () => {
	...
	d.forEach((v: Chat.Chat, _i: number) => {
	...
	<div class="overflow-y-auto bg-[#fafbfc] px-4 dark:bg-[#18181c] h-full">
		<section class="mb-4">
			...
			<div class="flex items-center justify-between space-x-1">
				<template v-for="(item, index) in vf" :key="item">
					<section
		...
		<div class="mb-4 flex justify-between items-center">
			<div class="flex justify-start items-center flex-wrap">
				<div class="pt-1 pr-1">
					<NPopover trigger="hover">
						...
						<div style="max-width: 240px">
							...
							>
								<div class="p-1" v-for="v in st.fileBase64" :key="v">
				...
				<div class="pt-1">
					...
					>
						<div style="display: flex">
							<SvgIcon icon="game-icons:bouncing-spring" />
							Shorten
						</div>
					</n-tag>
				</div>
