...
const subCustom = () => {
	if (chat.value.opt?.buttons == undefined) return;
	let i = getIndex(chat.value.opt?.buttons, {
		k: "CustomZoom::",
		n: t("mj.czoom"),
	});
	let obj = {
...
watch(
	...
	(n) => {
		...
		if (n == "mjReload" && actData.mjID == chat.value.mjID) {
			//&& actData.mjID==chat.value.mjID
...
<template>
	<div v-if="st.isLoadImg">
		<template v-if="chat.opt?.progress">
			<div
				v-if="chat.opt?.action == 'SHORTEN'"
				class="markdown-body"
				v-html="text"
			></div>
			<div
				v-else-if="chat.opt?.action != 'IMAGINE'"
				class="py-2 text-[#666] whitespace-pre-wrap"
			>
				{{ chat.opt?.promptEn }} (
				<span v-html="chat.opt?.action"></span>
				)
			</div>
			<NImage
				v-if="chat.opt.imageUrl"
				:src="st.uri_base64 ? st.uri_base64 : mjImgUrl(chat.opt.imageUrl)"
				class="rounded-sm"
			...
			<div v-if="chat.opt?.status == 'SUCCESS'" class="space-y-2">
				...
				<template
					v-else-if="
						chat.opt?.action === 'UPSCALE' || 'DESCRIBE' === chat.opt?.action
					"
				></template>
				<template v-else>
					<div class="flex space-x-2">
						<NButton type="primary" @click="sub('UPSCALE', 1)" size="small">
							U1
						</NButton>
						<NButton type="primary" @click="sub('UPSCALE', 2)" size="small">
							U2
						</NButton>
						<NButton type="primary" @click="sub('UPSCALE', 3)" size="small">
							U3
						</NButton>
						<NButton type="primary" @click="sub('UPSCALE', 4)" size="small">
							U4
						</NButton>
					</div>
					<div class="flex space-x-2">
						<NButton type="warning" @click="sub('VARIATION', 1)" size="small">
							V1
						</NButton>
						<NButton type="warning" @click="sub('VARIATION', 2)" size="small">
							V2
						</NButton>
						<NButton type="warning" @click="sub('VARIATION', 3)" size="small">
							V3
						</NButton>
						<NButton type="warning" @click="sub('VARIATION', 4)" size="small">
							V4
						</NButton>
						<NButton
							type="warning"
		...
		<NModal
			...
			preset="card"
			:title="$t('mjchat.redrawEditing')"
			style="max-width: 800px"
			@close="st.isShow = false"
		>
			<aiCanvas
				:chat="chat"
				:base64="st.uri_base64"
				v-if="st.isShow"
				@success="maskOk"
			/>
		</NModal>
		<NModal
			v-model:show="st.isCustom"
			preset="card"
			:title="$t('mj.customTitle')"
			style="max-width: 600px"
			@close="st.isCustom = false"
		>
			<n-input
				type="textarea"
				v-model:value="st.customText"
				round
				maxlength="2000"
				show-count
	...
	<div
		v-else
		class="w-[200px] h-[150px] flex flex-col justify-center items-center"
	>
		<div class="p-4">{{ $t("mjchat.loading") }}</div>

		<NButton type="primary">
			<a :href="chat.opt?.imageUrl" target="_blank">
				{{ $t("mjchat.openurl") }}
			</a>
		</NButton>
	</div>
