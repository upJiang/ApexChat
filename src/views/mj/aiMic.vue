...
<template>
	<template v-if="!st.isGo">
		<NButton @click="start()" type="primary" block round>
			<template #icon><SvgIcon icon="bi:mic" /></template>
			{{ $t("mj.mStart") }}
		</NButton>
	</template>
	<n-button-group v-else>
		...
		<NButton type="primary" @click="pause()" v-if="st.start == 1">
			<template #icon><SvgIcon icon="bi:pause-circle" /></template>
			{{ $t("mj.mPause") }}
		</NButton>
		...
		<template v-if="stat.duration > 0">
			<NButton type="info" @click="stopAdnRecord()" v-if="!isMobile">
				<template #icon><SvgIcon icon="bi:bootstrap-reboot" /></template>
				{{ $t("mj.mRecord") }}
			</NButton>

			<NButton @click="play()" type="info">
				<template #icon><SvgIcon icon="bi:play-circle" /></template>
				{{ $t("mj.mPlay") }}
			</NButton>
			<NButton @click="cancal()" type="info">
				<template #icon><SvgIcon icon="ri:close-circle-line" /></template>
				{{ $t("mj.mCanel") }}
			</NButton>

			<NButton type="primary" @click="send()">
				<template #icon><SvgIcon icon="ri:send-plane-fill" /></template>
				{{ $t("mj.mSent") }}
				<span class="w-[30px]" v-if="stat.duration > 0">
					{{ stat?.duration.toFixed(1) }}s
				</span>
			</NButton>
		</template>
	</n-button-group>
