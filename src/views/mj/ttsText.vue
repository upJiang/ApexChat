...
const download = () => {
	...
	a.download =
		props.chat?.model +
		"_" +
		props.chat.opt?.lkey?.replace(/\:/gi, "-") +
		".mp3";
...
<template>
	<div
		class="markdown-body"
		:class="[props.isW ? 'border-t border-neutral-400/25 ' : '']"
	...
	>
		...
		>
			<div class="flex justify-start items-center flex-1" @click="go">
				<span v-html="chat.opt?.duration?.toFixed(2)"></span>
				s
				<div class="rotate-90 cursor-pointer">
					<SvgIcon icon="svg-spinners:wifi" v-if="st.isLoad == 1" />
					<SvgIcon icon="mdi:wifi" v-else />
				</div>
			</div>
