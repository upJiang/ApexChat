...
import { NButton, NImage, NModal, useMessage } from "naive-ui";
...
const load = async () => {
	...
	if (
		chat.value.model != "dall-e-3" ||
		!chat.value.myid ||
		!chat.value.opt?.imageUrl
	) {
	...
	try {
		if (chat.value.opt?.imageUrl) {
			...
			if (!base64) {
				const ubase64 = await url2base64(
	...
	<div>
		<div v-if="st.isLoadImg">
			<NImage
				...
				:src="st.uri_base64 ? st.uri_base64 : chat.opt.imageUrl"
				class="rounded-sm"
				:class="[isMobile ? '' : '!max-w-[500px]']"
			/>
		</div>
		<div
			v-else-if="chat.opt?.imageUrl"
