...
<template>
	...
	>
		...
		>
			<p
				class="text-xs group text-[#b4bbc4] flex justify-start items-center space-x-2"
			...
			>
				...
				<template v-if="chat?.opt?.status == 'SUCCESS'">
					...
					<a
						:href="mjImgUrl(chat?.opt?.imageUrl)"
						class="hidden group-hover:block active cursor-pointer underline"
						target="_blank"
					>
						{{ $t("mj.ulink") }}
					</a>
