...
import { gptConfigStore, useAppStore, useChatStore } from "@/store";
...
async function handleSelect(item: Chat.History) {
	if (isActive(item.uuid)) return;
	...
	if (item.isGpts) {
		gptConfigStore.setMyData({
			gpts: { name: item.modelName || item.title },
			gptsType: item.gptsType,
		});
	} else {
		gptConfigStore.setMyData({
			gpts: undefined,
